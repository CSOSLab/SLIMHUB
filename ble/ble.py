...
def notify_callback(dev, sender, data):
    ...
    if len(data) == 32:
        ...
        save_file_at_dir(lookup[0][str(dev.address)+str(sender.handle)],
                         str(datetime.now().strftime("%Y.%m.%d"))+".txt", tmp_data)
    elif len(data) == 1:
        save_file_at_dir(lookup[0][str(dev.address)+str(sender.handle)], str(
            datetime.now().strftime("%Y.%m.%d"))+".txt", int.from_bytes(data, "little"))
    else:
        # print(f"{sender}: {data}")
...
def save_file_at_dir(dir_path, filename, file_content, mode='a'):
    with open(os.path.join(dir_path, filename), mode) as f:
        if "list" in str(type(file_content)):
            ...
            f.write(datetime.now().strftime("%X") +
                    ":    "+str(file_content)+"\n")
