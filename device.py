...
class DeviceManager:
    ...
    # Process functions ------------------------------------------------------------
    def _save_file_at_dir(self, device, handle, file_content, mode='a'):
        ...
        with open(os.path.join(dir_path, filename), mode) as f:
            if len(file_content) == 1:
                ...
                # f.write(current_time.strftime("%Y-%m-%d %H:%M:%S")+","+str(grideye_msg)+"\n")
                grideye_msg = grideye_msg.replace("(", "").replace(")", "")
                grideye_msg = grideye_msg.replace(",,", ",")
                f.write(current_time.strftime("%Y-%m-%d %H:%M:%S")+","+grideye_msg+"\n")
                mqtt_msg_dict = {}
                mqtt_msg_dict.update(SH_ID=self.mqtt.sh_id)
                mqtt_msg_dict.update(location=self.device_location)
                mqtt_msg_dict.update(time=current_time.strftime("%Y-%m-%d %H:%M:%S"))
                ...
                # f.write(current_time.strftime("%Y-%m-%d %H:%M:%S")+","+str(aat_msg)+"\n")
                aat_msg = aat_msg.replace("(", "").replace(")", "")
                aat_msg = aat_msg.replace(",,", ",")
                f.write(current_time.strftime("%Y-%m-%d %H:%M:%S")+","+aat_msg+"\n")
                ...
                # print("[LOG] : " + current_time.strftime("%X")+","+log_msg)
                ...
                f.write(current_time.strftime("%Y-%m-%d %H:%M:%S")+","+file_msg+"\n")
    
    def _process_data(self):
        while True:
            address, handle, data = self.queue_data.get()

            if len(data) < 37:
                self._save_file_at_dir(self.connected_devices[address], handle, data)
                
    def _process_sound(self):
        window_hop = int((self.sound_unit_samples * self.sound_clip_length_sec)/2)

        while True:
            address, handle, data = self.queue_sound.get()

            current_device = self.connected_devices[address]
            current_time = datetime.now()

            wav_path = os.path.join(current_device.path[str(handle)],"wavfiles",str(current_time.strftime("%Y-%m-%d")))
            
            os.makedirs(os.path.join(current_device.path[str(handle)],"logs"), exist_ok=True)
            os.makedirs(os.path.join(current_device.path[str(handle)],"raw"), exist_ok=True)

            f_logs = open(os.path.join(current_device.path[str(handle)],"logs",str(current_time.strftime("%Y-%m-%d"))+".txt"), 'a')
            f_raw = open(os.path.join(current_device.path[str(handle)],"raw",str(current_time.strftime("%Y-%m-%d"))+".txt"), 'a')

            # Mic stop trigger packet: save wav and clear buffer
            if data == b'\xff\xff\xff\xff':
                os.makedirs(wav_path, exist_ok=True)
                snd.save_wav(os.path.join(wav_path, str(current_time.strftime("%Y-%m-%d %H:%M:%S"))+".wav"), current_device.pcm_buffer_save, self.sound_sample_rate)
                current_device.pcm_buffer.clear()
                current_device.pcm_buffer_save.clear()
                current_device.voting_buffer.clear()
                continue
            
            pcm = snd.adpcm_decode(data)

            current_device.pcm_buffer.extend(pcm)
            current_device.pcm_buffer_save.extend(pcm)
            
            # Save wav files every 'sound_clip_length_sec' sec
            if len(current_device.pcm_buffer_save) >= (self.sound_sample_rate * self.sound_clip_length_sec):
                os.makedirs(wav_path, exist_ok=True)
                snd.save_wav(os.path.join(wav_path, str(current_time.strftime("%Y-%m-%d %H:%M:%S"))+".wav"), current_device.pcm_buffer_save, current_device.sound_sample_rate)
                current_device.pcm_buffer_save.clear()

            # Inference every 'window_hop' sec
            if (len(current_device.pcm_buffer) >= (current_device.sound_unit_samples)):
                # Preprocess and inference
                mfcc = snd.get_mfcc(current_device.pcm_buffer[:self.sound_unit_samples], sr=self.sound_sample_rate, n_mfcc=32, n_mels=64, n_fft=1000, n_hop=500)

                result = tflite.inference(self.env_interpreter, mfcc)

                # Save raw inference result
                time = str(current_time.strftime("%Y-%m-%d %H:%M:%S"))
                f_raw.write(time+','+','.join(result.astype(str))+'\n')

                # Postprocessing
                current_device.voting_buffer.append(result)
                if len(current_device.voting_buffer) == self.voting_buffer_len:
                    # Todo: Postprocessing
                    buf = np.array(current_device.voting_buffer).swapaxes(0, 1)
                    ...
                    for idx in idxs:
                        ...
                        mqtt_msg_dict = {}
                        mqtt_msg_dict.update(SH_ID=self.mqtt.sh_id)
                        mqtt_msg_dict.update(location=current_device.device_location)
                        mqtt_msg_dict.update(time=current_time.strftime("%Y-%m-%d %H:%M:%S"))
                        ...
                        msgq_payload_sound_str = msgq_payload_sound_str+"SJK,"+str(current_device.device_location)+","+str(Device.classlist[idx])+","+str(idx)+","+str('%.2f'%mean)+",,,,,,,"+"\n"
                        self.msgq.send(msgq_payload_sound_str, MSGQ_TYPE_SOUND)
                        
                        # Print inference result
                        print(Device.classlist[idx], counts[idx], '%.2f'%mean)

                    current_device.voting_buffer = current_device.voting_buffer[5:]

                current_device.pcm_buffer = current_device.pcm_buffer[window_hop:]
